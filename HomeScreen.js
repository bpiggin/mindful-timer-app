...
import { Text, View, TextInput, Keyboard, TouchableOpacity, StatusBar } from 'react-native';
...
class DurationEntry extends React.Component {
  constructor(props) {
    super(props)
    this.state = {duration: ''}
  }
  ...
  onBeginPressed() {
    if (this.state.duration != "" && +this.state.duration > 0) {
      ...
      this.props.navigation.navigate('Settle', {duration: parseInt(this.state.duration), delay: 3})
    }
  }
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.titleText}>How many minutes are you meditating for?</Text>
        <TextInput  
          placeholder="     "
